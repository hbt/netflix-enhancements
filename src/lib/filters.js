...
var Filters = {
  ...
  toggle: function()
  {
    ...
    {
      var id = parseInt(/\d+/.exec($(v).children().first().attr('id'))[0])
  ...
  boot: function()
  {
    ...
    var html = '<div>'
    html += '<label for="filter-rated">Filter Rated</label> <input name="filter-rated" type="checkbox" class="filters filter-rated" checked/>'
    html += '<label for="filter-not-interested">Filter Not Interested</label> <input name="filter-not-interested" type="checkbox" class="filters filter-not-interested" checked/>'
